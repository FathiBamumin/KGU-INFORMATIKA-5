...
const GamifikasiPage = () => {
  ...
  const [levels, setLevels] = useState<Level[]>([]);  const [loading, setLoading] = useState(true);
  ...
  async function fetchData() {
    try {
      // Gunakan method yang benar dari api.ts
      const [studentsResponse, statsResponse] = await Promise.all([
        gamificationApi.getAll(),      // Ambil semua data gamifikasi
        gamificationApi.getStats()      // Ambil statistik
      ]);
